...
Firmware for the BananaGuard edge node: camera-based banana-weevil detection
with frame-differencing ROI, INT8 MobileNetV2 inference (TFLite-Micro), soil
gating, solenoid actuation with hard safety lockout, LoRaWAN telemetry and a
30-minute deep-sleep duty cycle.

**Member 2 deliverable, Weeks 4–13. Target: ESP-IDF ≥ 5.1, ESP32 (WROOM-32).**
...
| Frame-diff ROI | `roi_diff.[ch]` | 64×48 grayscale reference in RTC RAM; diff → bbox → padded square crop; skips inference when nothing moved |
| Inference | `inference.h/.cc`, `model_data.h/.cc` | TFLite-Micro, INT8 MobileNetV2 as C array; returns class+confidence, rejects negative class below 60 % |
...
| LoRaWAN telemetry | `lora_telemetry.[ch]` | OTAA, US915 sub-band 2, SF10, 9-byte binary uplink (payload spec in header) |
...
2. **LoRaWAN stack** — clone [ttn-esp32](https://github.com/manuelbl/ttn-esp32)
   ...
   payload bytes. Set the OTAA keys in `app_config.h`.
...
2. `actuation_spray()` — re-checks daily limit, minimum gap and fault flag
   internally (does not trust the caller),
...
After a cold boot every guard fails safe: counters reset to zero and the node
needs one full 30-min window of detections before it can spray again.
